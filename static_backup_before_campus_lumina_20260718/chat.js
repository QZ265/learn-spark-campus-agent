...
function welcomeHtml(){
  return `
    <article class="message assistant">
      ...
      <div class="bubble">
        <strong>你好，我是课程学习智能体。</strong>
        <p>请选择课程后提问。已建立 LightRAG 索引的课程会严格限定在当前知识空间并返回教材引用。</p>
      </div>
    </article>
  `;
}
...
function renderQuickPrompts(){
  ...
  if(!wrap) return;
  const pageSize = 5;
  const start = (promptPage * pageSize) % QUICK_PROMPTS.length;
  const items = Array.from({length: pageSize}).map((_, index) => QUICK_PROMPTS[(start + index) % QUICK_PROMPTS.length]);
